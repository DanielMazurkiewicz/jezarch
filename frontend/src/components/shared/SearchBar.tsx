        ...
        setCriteria(currentCriteria => {
            ...
            if (field === 'field') {
                ...
                criterion.value = newType === 'boolean' ? true : (newType === 'select' && newCondition === 'ANY_OF' ? [] : '');
                 ...
                 else if (currentFieldDef?.type === 'select' && value === 'ANY_OF') criterion.value = [];
                 else if (criterion.value === undefined || criterion.value === null || Array.isArray(criterion.value)) criterion.value = '';
             }
    ...
    const handleSearchClick = () => {
        const finalQuery: SearchQuery = criteria.map(crit => {
            ...
            switch (fieldDef.type) {
                ...
                case 'select':
                    if (crit.condition === 'ANY_OF') { if (!Array.isArray(parsedValue)) parsedValue = [parsedValue].filter(v => v !== null && v !== undefined && v !== ''); if (parsedValue.length === 0) isValid = false; } // Filter out empty/null values before checking length
            ...
            // We don't need the specific subtypes here, just ensure the object matches the union structure.
            const queryElement = {
                field: crit.field!,
        ...
        <div className="p-4 border rounded-lg bg-card space-y-3">
            {criteria.map((criterion) => (
                <div key={criterion._key} className="flex flex-wrap items-end gap-2 p-2 border rounded bg-background">
                    {/* Field */} <div className='flex-grow min-w-[150px]'> <Label htmlFor={`field-${criterion._key}`} className='text-xs'>Field</Label> <Select value={criterion.field} onValueChange={(value) => handleCriterionChange(criterion._key!, 'field', value)}> <SelectTrigger id={`field-${criterion._key}`}><SelectValue placeholder="Select field..." /></SelectTrigger> <SelectContent> {fields.map(f => ( <SelectItem key={f.value} value={f.value}>{f.label}</SelectItem> ))} </SelectContent> </Select> </div>
                    {/* NOT */} <div className='flex items-center space-x-1 self-center pt-4'> <input type="checkbox" id={`not-${criterion._key}`} checked={criterion.not || false} onChange={(e) => handleCriterionChange(criterion._key!, 'not', e.target.checked)} className='h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary' /> <Label htmlFor={`not-${criterion._key}`} className='text-xs font-medium'>NOT</Label> </div>
                    {/* Condition */} {criterion.field && ( <div className='flex-grow min-w-[120px]'> <Label htmlFor={`condition-${criterion._key}`} className='text-xs'>Condition</Label> <Select value={criterion.condition} onValueChange={(value) => handleCriterionChange(criterion._key!, 'condition', value)} disabled={!criterion.field} > <SelectTrigger id={`condition-${criterion._key}`}><SelectValue /></SelectTrigger> <SelectContent> {(conditionsByType[getFieldType(criterion.field)] || []).map(c => ( <SelectItem key={c.value} value={c.value}>{c.label}</SelectItem> ))} </SelectContent> </Select> </div> )}
                    {/* Value */} {criterion.field && ( <div className='flex-grow min-w-[180px]'> <Label htmlFor={`value-${criterion._key}`} className='text-xs'>Value</Label> { getFieldType(criterion.field) === 'boolean' ? ( <Select value={String(criterion.value ?? true)} onValueChange={(value) => handleCriterionChange(criterion._key!, 'value', value === 'true')} disabled={!criterion.field} > <SelectTrigger id={`value-${criterion._key}`}><SelectValue /></SelectTrigger> <SelectContent> <SelectItem value="true">True</SelectItem> <SelectItem value="false">False</SelectItem> </SelectContent> </Select> ) : getFieldType(criterion.field) === 'select' && criterion.condition !== 'ANY_OF' ? ( <Select value={String(criterion.value ?? '')} onValueChange={(value) => handleCriterionChange(criterion._key!, 'value', value)} disabled={!criterion.field} > <SelectTrigger id={`value-${criterion._key}`}><SelectValue placeholder="Select value..."/></SelectTrigger> <SelectContent> {(getFieldOptions(criterion.field) || []).map(opt => ( <SelectItem key={String(opt.value)} value={String(opt.value)}>{opt.label}</SelectItem> ))} </SelectContent> </Select> ) : ( <Input id={`value-${criterion._key}`} type={getFieldType(criterion.field) === 'number' ? 'number' : 'text'} value={criterion.value as string | number ?? ''} onChange={(e) => handleCriterionChange(criterion._key!, 'value', e.target.value)} placeholder={getFieldType(criterion.field) === 'date' ? 'YYYY-MM-DD' : 'Enter value...'} disabled={!criterion.field} /> ) } </div> )}
                    {/* Remove */} <Button variant="ghost" size="icon" onClick={() => handleRemoveCriterion(criterion._key!)} className='self-end text-muted-foreground hover:text-destructive' title='Remove criterion' disabled={criteria.length <= 1} > <Trash2 className="h-4 w-4" /> </Button>
                </div>
            ))}
            {/* Actions */} <div className="flex justify-between items-center pt-2"> <Button type="button" variant="outline" onClick={handleAddCriterion} size="sm"> <PlusCircle className="mr-2 h-4 w-4" /> Add </Button> <Button type="button" onClick={handleSearchClick} disabled={isLoading}> {isLoading && <LoadingSpinner size='sm' className='mr-2' />} <Search className="mr-2 h-4 w-4" /> Search </Button> </div>
        </div>
    );
};
