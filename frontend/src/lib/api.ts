...
// --- API Function Exports --- (Remain the same as previous version)
// API Status
...
const logout = (token: string) => fetchApi<string>("/user/logout", "POST", null, token);
...
// Note
const createNote = (noteData: NoteInput, token: string) => fetchApi<{ message: string }>('/note', 'PUT', noteData, token);
const getNoteById = (noteId: number, token: string) => fetchApi<Note>(`/note/id/${noteId}`, 'GET', null, token);
const updateNote = (noteId: number, noteData: NoteInput, token: string) => fetchApi<{ message: string }>(`/note/id/${noteId}`, 'PATCH', noteData, token);
const deleteNote = (noteId: number, token: string) => fetchApi<{ message: string }>(`/note/id/${noteId}`, 'DELETE', null, token);
const getNotesByLogin = (login: string, token: string) => fetchApi<Note[]>(`/notes/by-login/${login}`, 'GET', null, token);
const searchNotes = (searchRequest: SearchRequest, token: string) => fetchApi<SearchResponse<Note>>("/notes/search", "POST", searchRequest, token);
// Signature Component
...
const disableArchiveDocument = (id: number, token: string) => fetchApi<void>(`/archive/document/id/${id}`, 'DELETE', null, token);
