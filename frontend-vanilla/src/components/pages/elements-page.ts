...
import '../../ui/app-badge';
import '../../shared/search-bar'; // Keep import for definition loading
import '../../ui/pagination'; // Corrected import path
import '../../ui/error-display';
import '../../ui/loading-spinner';
import '../../signatures/element-list'; // Keep import for definition loading
import '../../signatures/element-form'; // Keep import for definition loading

// Import component types explicitly
// Keep type imports assuming definitions are loaded elsewhere or will be loaded
// NOTE: Type-only imports for components defined elsewhere. Assume definitions are loaded.
import type { SearchBar, SearchFieldOption, SearchCondition } from '../shared/search-bar';
...
export class ElementsPage extends BaseComponent {
    ...
    protected get styles(): string {
     ...
     protected get template(): string {
        ...
        const canCreate = !!comp && this.auth.isAuthenticated; // Check auth for create button
    ...
    disconnectedCallback(): void {
     ...
     attributeChangedCallback(name: string, oldValue: string | null, newValue: string | null) {
         // No need for super.attributeChangedCallback as BaseComponent doesn't implement it
         if (name === 'param-componentid' && oldValue !== newValue && this.isConnected) {
             ...
             if(searchBar) searchBar.reset(); // Reset search bar state
    ...
    private async fetchParentComponent(id: number) {
     ...
     private updateSearchFields() {
         ...
         if(searchBar) {
            searchBar.fields = [
              ...
              console.warn("ElementsPage: Search bar not found to update fields.");
         }
     }
     private async fetchElements() {
        ...
        try {
            ...
            // Use type assertion for conditions like IS_NULL/IS_NOT_NULL if needed
            // Adjusted filter logic to correctly handle IS_NULL/IS_NOT_NULL checks alongside value checks
            const validSearchQuery = this.elementSearchQuery.filter(q => {
                 if (!q || !q.condition) return false;
                 const condition = q.condition as SearchCondition; // Use SearchCondition type from SearchBar import
                 // Keep criteria if condition doesn't require a value
                 if (condition === 'IS_NULL' || condition === 'IS_NOT_NULL') return true;
                 // Otherwise, keep criteria only if a non-empty value is provided
                 const valueStr = String(q.value ?? '');
    ...
    private setLoadingState(isLoading: boolean) {
        ...
        // Use qsOptional and check for existence before accessing properties
        ...
        if (!isLoading) this.updateListAndPagination();
            ...
            this.qsOptional<ElementList>('#element-list')?.toggleAttribute('hidden', true);
            this.qsOptional<Pagination>('#pagination')?.toggleAttribute('hidden', true);
    ...
    private setErrorState(error: string | null) {
         ...
         if (listErrorDisplay) {
             ...
             listErrorDisplay.hidden = !error || this.isLoadingElements || this.isLoadingParent;
         }
    ...
    addEventListeners() {
         ...
         const backButton = this.qsOptional<AppButton>('#back-button');
         if (backButton) this.registerListener(backButton, 'click', () => router.navigate('/signatures'));


         const createButton = this.qsOptional<AppButton>('#create-button');
         if (createButton) this.registerListener(createButton, 'click', this.handleCreateElement);

         const searchBar = this.qsOptional<SearchBar>('#search-bar');
         if (searchBar) this.registerListener(searchBar, 'search', this.handleElementSearch);

         const pagination = this.qsOptional<Pagination>('#pagination');
         if (pagination) this.registerListener(pagination, 'pagechange', this.handleElementPageChange);

         const elementList = this.qsOptional<ElementList>('#element-list');
         if (elementList) {
            this.registerListener(elementList, 'edit', this.handleEditElement);
            this.registerListener(elementList, 'delete', this.handleDeleteElement);
         }
    }
    ...
    private handleEditElement = (event: Event) => {
        ...
        if (!this.parentComponent || !this.auth.isAuthenticated) {
         ...
         if (!this.auth.isAdmin) {
            showToast("Admin privileges required to edit elements.", "error"); return;
         }
     ...
     private handleCreateElement = () => {
         ...
         if (!this.auth.isAdmin) {
            showToast("Admin privileges required to create elements.", "error"); return;
         }
