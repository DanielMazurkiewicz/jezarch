...
// Assuming backend exports enums directly or within models
import { SignatureComponentIndexType as BackendSignatureComponentIndexType } from '../../../backend/src/functionalities/signature/component/models';
import { ArchiveDocumentType as BackendArchiveDocumentType } from '../../../backend/src/functionalities/archive/document/models';
...
// --- Signature Component ---
// Use the backend enum directly if possible, otherwise re-declare
const FrontendSignatureComponentIndexType = z.enum(BackendSignatureComponentIndexType.options);
...
export const createSignatureComponentFormSchema = z.object({
    ...
    index_type: FrontendSignatureComponentIndexType, // Use the enum
});
...
export const elementFormSchema = z.object({
    ...
    // Allow empty string for index, backend interprets as 'auto'
...
// --- Archive Document ---
const FrontendArchiveDocumentType = z.enum(BackendArchiveDocumentType.options);
...
export const createArchiveDocumentFormSchema = z.object({
    ...
    type: FrontendArchiveDocumentType,
    // Signatures handled by SignatureSelector component, not directly in form data schema typically
    // topographicSignatureElementIds: z.array(z.array(z.number().int().positive())).optional().default([]), // Keep for reference maybe
    ...
    tagIds: z.array(z.number().int().positive()).optional().default([]), // For association
