...
// NOTE: If TS errors persist for these imports (TS6307), ensure your tsconfig.json
// 'include' or 'files' array correctly covers the backend source directory,
// or adjust the relative paths if the directory structure is different.
import type {
    UserCredentials,
    User,
    UserRole,
...
async function fetchApi<T>(
    ...
    // Always accept JSON unless it's a specific endpoint like ping
    ...
    if (!response.ok) {
        ...
        try {
             ...
             try {
                  // Attempt to parse JSON, but fallback to text if parsing fails or content isn't JSON
                  if (response.headers.get('content-type')?.includes('application/json')) {
                      const parsed = JSON.parse(textResponse);
                      if (typeof parsed === 'object' && parsed !== null && parsed.message) {
                          errorData = { message: parsed.message }; // Use message from JSON error
                      } else {
                           errorData = { message: textResponse || `API Error ${response.status}` }; // Fallback to text
                      }
                  } else if (textResponse) { // Handle non-JSON error response
                      errorData = { message: textResponse };
                  }
             } catch (e) {
                  // Parsing failed, use the raw text response if available
    ...
    if (endpoint.endsWith('/ping') && response.headers.get('content-type')?.includes('text/plain')) {
        return await response.text() as T;
    }

    // Handle 204 No Content (or potentially empty body)
    if (response.status === 204 || response.headers.get('content-length') === '0') {
        return { success: true } as T; // Return success indicator or adjust as needed
    }

    // For all other successful responses, expect JSON
    try {
